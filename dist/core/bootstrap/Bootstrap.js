...
export class Bootstrap {
    ...
    static async start() {
        ...
        client.on(Events.MessageDelete, async (message) => {
            try {
                ...
                const deleteHooks = this.manifests
                    ...
                    .map(m => m.onMessageDelete(message));
        ...
        client.on(Events.MessageReactionAdd, async (reaction, user) => {
            try {
                const reactionAddHooks = this.manifests
                    ...
                    .map(m => m.onReactionAdd(reaction, user));
        ...
        client.on(Events.MessageReactionRemove, async (reaction, user) => {
            try {
                const reactionRemoveHooks = this.manifests
                    ...
                    .map(m => m.onReactionRemove(reaction, user));
        ...
        try {
            ...
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('LOGIN TIMEOUT: client.login() did not resolve within 30 seconds')), 30_000));
    ...
    static async handleRestartResume(client) {
