...
export async function handleMessage(message) {
    ...
    if (authority < AuthorityLevel.Owner) {
        ...
        if (maintenance.enabled) {
            ...
            await respond.transientWarning(`**Maintenance in Progress**\n` +
                `The bot is currently undergoing scheduled maintenance.\n\n` +
                `• **Reason:** ${maintenance.reason}\n` +
                `• *Commands are temporarily reserved for developers. Please check back shortly!*`, 8000);
            return;
        }
    }
    ...
    if (message.reference?.messageId) {
        try {
            let referenced = message.referencedMessage;
            if (!referenced) {
    ...
    const ctx = {
        ...
        respond,
        async canExecute(cmdName) {
            const targetCmd = resolveCommand(cmdName);
            if (!targetCmd || !targetCmd.enabled)
                return false;
    ...
    try {
        ...
        let outcome = 'success';
        if (replyType === 'warning')
            outcome = 'warning';
        else if (replyType === 'info')
            outcome = 'info';
        ...
        if (error instanceof BotError) {
            await respond.error(userMsg).catch(() => { });
        }
        else {
            await respond.error('An unexpected error occurred.').catch(() => { });
        }
        recordCommandLog(message.client, {
            guildId: message.guild.id, guildName: message.guild.name, channelId: guildChannel.id, channelName: guildChannel.name,
            userId: message.author.id, userTag: message.author.tag, commandName: command.name, aliasUsed: parsed.aliasUsed,
