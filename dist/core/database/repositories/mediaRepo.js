...
const mediaChannelsCache = new Map(); // guildId -> Set<channelId>
export async function addMediaChannel(guildId, channelId) {
    const db = getDb();
...
export async function removeMediaChannel(guildId, channelId) {
    const db = getDb();
    const result = await db `
  ...
  `;
    if (result.count > 0) {
        const cached = mediaChannelsCache.get(guildId);
        if (cached)
            cached.delete(channelId);
        return true;
    }
    return false;
}
export async function getMediaChannels(guildId) {
    const db = getDb();
    const rows = await db `
...
export async function isMediaChannel(guildId, channelId) {
...
export async function setMediaAutoThread(guildId, enabled) {
    const db = getDb();
    await db `
...
export async function getMediaAutoThread(guildId) {
    const db = getDb();
    const rows = await db `
...
//# sourceMappingURL=mediaRepo.js.map