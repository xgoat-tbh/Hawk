...
import { addIgnore, removeIgnore, getIgnoreEntries, } from '../../core/database/repositories/ignoreRepo.js';
    ...
    async execute(ctx) {
        ...
        if (sub === 'list') {
            ...
            const lines = Array.from(groupedMap.values()).map((g) => {
                const targetStr = g.entityId === 'all' || g.entityId === '*'
                    ? '**All Targets**'
                    : g.entityType === 'role'
        ...
        if (sub === 'remove' || sub === 'delete') {
            ...
            const removed = await removeIgnore(guild.id, target.type, target.id, scopeType, scopeId, modeArg);
            if (removed) {
...
async function resolveTarget(input, guild) {
    const clean = input.toLowerCase();
    if (clean === 'all' || clean === '*') {
