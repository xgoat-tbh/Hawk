...
export async function buildInfoV2Embed(client, _guild, prefix, userId) {
    ...
    const content = `• **Bot:** **${botUser?.username ?? 'Amo'}** (\`${botTag}\`)\n` +
    ...
    const helpBtn = new ButtonBuilder()
        ...
        .setLabel('Help')
        .setStyle(ButtonStyle.Secondary);
    const actionRow = new ActionRowBuilder().addComponents(refreshBtn, helpBtn);
    return ui.standard({
...
export async function handleInfoInteraction(interaction) {
