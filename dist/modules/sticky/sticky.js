...
import { getSticky, setSticky, deleteSticky, updateStickyMessageId, getStickiesForGuild, } from '../../core/database/repositories/stickyRepo.js';
...
export default defineCommand({
    ...
    async execute(ctx) {
        ...
        if (!stickyContent && message.reference?.messageId) {
            ...
            if (referencedMsg) {
                ...
                if (referencedMsg.content)
                    parts.push(referencedMsg.content);
        ...
        if (!stickyContent) {
            await respond.error(`Usage: \`${parsed.prefix}sticky <message...>\` or reply to a message with \`${parsed.prefix}sticky\`.\nSubcommands: \`edit\`, \`remove\`, \`refresh\`, \`list\`.`);
            return;
        }
...
async function handleStickyEdit(ctx, newContent) {
...
async function handleStickyRemove(ctx) {
    const { guild, channel, message, respond, member } = ctx;
    const existingSticky = await getSticky(guild.id, channel.id);
    if (!existingSticky) {
...
async function handleStickyRefresh(ctx) {
    const { guild, channel, message, respond, member } = ctx;
    const existingSticky = await getSticky(guild.id, channel.id);
    if (!existingSticky) {
        await respond.error('No active sticky configuration found for this channel.');
