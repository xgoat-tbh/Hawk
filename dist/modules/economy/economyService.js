...
function mapBalance(row) {
    return {
        ...
        dailyLast: row.daily_last ? new Date(row.daily_last) : null,
        dailyStreak: Number(row.daily_streak ?? 0),
        workLast: row.work_last ? new Date(row.work_last) : null,
...
export async function ensureBalance(guildId, userId) {
    const db = getDb();
...
export async function addCash(guildId, userId, amount) {
    if (amount <= 0)
        return;
    const db = getDb();
...
export async function removeCash(guildId, userId, amount) {
    if (amount <= 0)
        return true;
    const db = getDb();
...
export async function transferCash(guildId, fromUserId, toUserId, amount) {
    if (amount <= 0)
...
export async function deductFundsPreferCash(guildId, userId, amount) {
    if (amount <= 0)
        return { deductedFromCash: 0, deductedFromBank: 0 };
    const db = getDb();
    return await db.begin(async (tx) => {
        const rows = await tx `
      SELECT cash, bank FROM economy_balances
...
export async function getLeaderboard(guildId, sortBy = 'net', page = 1, pageSize = 10) {
    const db = getDb();
  ...
  `;
    ...
    const rows = await db.unsafe(`SELECT user_id, cash, bank, (cash + bank) as net_worth,
     ROW_NUMBER() OVER (ORDER BY ${orderClause}) as rank
     FROM economy_balances
     WHERE guild_id = $1 AND (cash > 0 OR bank > 0)
     ORDER BY ${orderClause}
     LIMIT $2 OFFSET $3`, [guildId, pageSize, offset]);
...
export async function addMoneyToRole(guildId, memberIds, amount, target) {
...
export async function setCooldownTimestamp(guildId, userId, field) {
    const db = getDb();
    await db `
    UPDATE economy_balances
...
// ── Audit Logging ─────────────────────────────────────────────
export async function logAuditAction(guildId, actorId, targetId, action, amount, details) {
    const db = getDb();
    await db `
...
export async function getAuditLog(guildId, page = 1, pageSize = 10) {
    const db = getDb();
    const offset = (page - 1) * pageSize;
...
export async function setBankCapacity(guildId, userId, capacity) {
    const db = getDb();
    await db `
...
export async function setGuildDefaultBankCapacity(guildId, capacity) {
    const db = getDb();
    const rows = await db `
    UPDATE economy_balances
...
export async function claimDaily(guildId, userId) {
    ...
    `;
        ...
        if (rows.length > 0) {
            ...
            lastClaim = rows[0].daily_last ? new Date(rows[0].daily_last) : null;
        }
        if (lastClaim) {
