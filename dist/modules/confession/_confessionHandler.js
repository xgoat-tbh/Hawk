...
import { getConfessionChannel, getConfessionConfig, setConfessionPanelMessageId, getAllConfessionConfigs, createConfessionRecord, } from '../../core/database/repositories/confessionRepo.js';
...
const activeConfessionPanels = new Map(); // guildId -> messageId
const confessionPanelChannels = new Set(); // channelId
const confessionPanelLocks = new Set(); // channelId
export function registerConfessionPanelChannel(channelId) {
...
export async function handleConfessionButton(interaction) {
    ...
    if (customId === 'confess_info') {
        await interaction.reply({
            content: '**Anonymous Confessions Info**\n\n' +
...
export async function handleConfessionPanelResurface(_message) {
    // Automatic resurfacing disabled per simple modal submission replacement flow
}
export async function initializeConfessionPanels(client) {
    try {
...
//# sourceMappingURL=_confessionHandler.js.map