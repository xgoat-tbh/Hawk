...
import { getWelcomeConfig, setGreetChannel, removeGreetPayload, setLeaveChannel, removeLeavePayload, } from '../../core/database/repositories/welcomeRepo.js';
import { buildWelcomeConfigPanel } from './welcomeUI.js';
import { WELCOME_VARIABLES_GUIDE, buildVariableContext, renderWelcomePayload, } from './welcomeEngine.js';
...
async function handleChannel(ctx, isGreet, args) {
...
async function handleTest(ctx, isGreet) {
    const { guild, respond, member } = ctx;
    const config = await getWelcomeConfig(guild.id);
    const channelId = isGreet ? config?.greetChannelId : config?.leaveChannelId;
