...
import { getWelcomeConfig, setGreetPayload, setLeavePayload, } from '../../core/database/repositories/welcomeRepo.js';
...
export async function handleWelcomeButton(interaction) {
    ...
    if (!customId.startsWith('welcome_') || interaction.replied || interaction.deferred)
        return;
...
export async function handleMemberLeave(member) {
    if (!member.guild)
        return;
    const config = await getWelcomeConfig(member.guild.id);
    if (!config?.leaveChannelId || !config.leavePayload)
        return;
    const textChannel = (await member.guild.channels.fetch(config.leaveChannelId).catch(() => null));
    if (!textChannel)
        return;
    try {
...
//# sourceMappingURL=_welcomeHandler.js.map