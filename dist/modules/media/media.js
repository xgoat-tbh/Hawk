...
import { addMediaChannel, removeMediaChannel, getMediaChannels, setMediaAutoThread, getMediaAutoThread, } from '../../core/database/repositories/mediaRepo.js';
...
async function handleChannelAdd(ctx, args) {
...
async function handleChannelRemove(ctx, args) {
    const { guild, respond, member } = ctx;
    if (args.length === 0) {
...
async function handleChannelList(ctx) {
...
async function handleThreadToggle(ctx, enabled) {
    const { guild, respond, member } = ctx;
