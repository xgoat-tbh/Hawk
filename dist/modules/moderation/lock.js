    ...
    async execute(ctx) {
        ...
        if (!isExplicitUnlock) {
            ...
            if (cleanArgs.length === 0) {
                await targetChannel.permissionOverwrites.edit(everyoneRole.id, permToEdit).catch(() => { });
                ...
                for (const [, overwrite] of targetChannel.permissionOverwrites.cache) {
                    if (overwrite.type === OverwriteType.Role && overwrite.id !== everyoneRole.id) {
                        await targetChannel.permissionOverwrites.edit(overwrite.id, permToEdit).catch(() => { });
                        count++;
                ...
                await targetChannel.permissionOverwrites.edit(targetRole.id, permToEdit);
            ...
            if (cleanArgs.length === 0) {
                await targetChannel.permissionOverwrites.edit(everyoneRole.id, permInherit).catch(() => { });
                logTarget = mentionRole(everyoneRole, guild);
                ...
                for (const [, overwrite] of targetChannel.permissionOverwrites.cache) {
                    if (overwrite.type === OverwriteType.Role && overwrite.id !== everyoneRole.id) {
                        await targetChannel.permissionOverwrites.edit(overwrite.id, permInherit).catch(() => { });
                        count++;
                        await new Promise(r => setTimeout(r, 50));
                    }
                }
                ...
                const targetRole = roleRes.value.role;
                await targetChannel.permissionOverwrites.edit(targetRole.id, permAllow);
                logTarget = mentionRole(targetRole, guild);
