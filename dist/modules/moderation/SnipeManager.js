...
export function recordDeletedMessage(message) {
    ...
    if (snipeCache.size >= MAX_CHANNELS && !snipeCache.has(channelId)) {
        ...
        if (firstKey !== undefined)
            snipeCache.delete(firstKey);
    }
    snipeCache.set(channelId, updated);
}
export function recordEditedMessage(oldMessage, newMessage) {
    // Only humans
    if (newMessage.author?.bot || newMessage.author?.system)
        return;
    if (!newMessage.guild || !newMessage.channel)
        return;
    ...
    if (editSnipeCache.size >= MAX_CHANNELS && !editSnipeCache.has(channelId)) {
        ...
        if (firstKey !== undefined)
            editSnipeCache.delete(firstKey);
    }
...
export function getEditSnipes(channelId) {
...
export function getEditSnipe(channelId, index = 1) {
