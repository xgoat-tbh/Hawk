...
export function buildNukeConfirmationPayload(channelId, authorId) {
    ...
    return ui.standard({
        ...
        text: `Are you sure you want to nuke <#${channelId}>?\n\n` +
            `• **Target Channel:** <#${channelId}>\n` +
            `• **Consequence:** Clones the channel (retaining permissions, topic, and position) and **permanently deletes** all message history.`,
        components: [row],
    });
}
export async function handleNukeInteraction(interaction) {
    const { customId, user, guild, channel } = interaction;
    if (!customId.startsWith('nuke_') || !guild || !channel)
        return;
    ...
    if (action === 'confirm') {
        ...
        try {
            ...
            const noticePayload = ui.standard({
                title: 'Channel Nuked',
                text: `This channel was recreated and cleared by ${mentionUser(user.id)}.\n\n` +
                    `• All messages were wiped while retaining channel permissions, topic, and position.`,
            });
            if ('send' in clonedChannel && typeof clonedChannel.send === 'function') {
                const noticeMsg = await clonedChannel.send(noticePayload).catch(() => null);
                if (noticeMsg) {
                    setTimeout(() => {
            ...
            logAuditAction({
                ...
                executor: interaction.member ?? user,
...
//# sourceMappingURL=_nukeHandler.js.map