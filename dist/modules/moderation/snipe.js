import { PermissionsBitField, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, ComponentType, } from 'discord.js';
import { defineCommand } from '../../types/command.js';
import { getSnipes, getEditSnipes, clearChannelSnipe, } from './SnipeManager.js';
import { HawkTheme } from '../../core/ui/theme.js';
import { ui } from '../../core/ui/index.js';
function buildSnipeEmbed(snipe, currentIndex, total) {
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(`**Author:** **${snipe.authorTag}** (\`${snipe.authorId}\`)\n` +
...
function buildEditSnipePayload(editSnipe, currentIndex, total) {
    ...
    if (editSnipe.attachments.length > 0) {
        sections.push(`**Attachments (${editSnipe.attachments.length}):**\n` +
...
function buildNavButtons(prefix, currentIndex, total) {
...
async function handleDeleteSnipe(ctx, args) {
    ...
    collector.on('collect', async (interaction) => {
        if (interaction.customId === 'snipe_prev') {
            ...
            if (currentIndex < snipes.length - 1)
                currentIndex++;
        }
...
async function handleEditSnipe(ctx, args) {
    const { channel, member, respond } = ctx;
