...
export function canExecutorManage(guild, executor, targetRole, targetMember) {
...
export function isRoleManageable(guild, role, executor) {
    if (role.managed)
        return false;
    if (role.id === guild.id)
...
export function isMemberManageable(guild, member, executor) {
    // Non-owners cannot manage Server Owner, but Server Owner can manage themselves
    if (member.id === guild.ownerId && executor?.id !== guild.ownerId)
        return false;
    const botMember = guild.members.me;
    if (!botMember)
        return false;
...
export async function toggleRoleForMember(guild, member, role, executor) {
...
export async function addRoleToMember(guild, member, role, executor) {
    if (!isRoleManageable(guild, role, executor) || !isMemberManageable(guild, member, executor)) {
        return 'skipped';
    }
    if (member.roles.cache.has(role.id)) {
...
export async function removeRoleFromMember(guild, member, role, executor) {
    if (!isRoleManageable(guild, role, executor) || !isMemberManageable(guild, member, executor)) {
        return 'skipped';
    }
    if (!member.roles.cache.has(role.id)) {
...
//# sourceMappingURL=roleHelpers.js.map