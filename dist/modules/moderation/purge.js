...
export default defineCommand({
    ...
    async execute(ctx) {
        ...
        if (parsed.args.length === 0) {
            await respond.error(`Usage: \`${parsed.prefix}purge [@user] <amount>\` or \`${parsed.prefix}purge <amount> [filter]\` or \`${parsed.prefix}purge role <@role>\``);
            return;
        }
        // ── Subcommand: role ──
...
async function handleMessagePurge(ctx) {
    ...
    if (!isNaN(firstAsNum) && firstAsNum > 0) {
        ...
        await respond.error(`Usage: \`${parsed.prefix}purge [@user] <amount>\` or \`${parsed.prefix}purge <amount> [bot|human|@user|embeds|links|images]\``);
        return;
    }
    ...
    while (deletedCount < amount) {
        ...
        const fetched = await textChannel.messages.fetch(fetchOptions).catch(() => new Map());
        if (fetched.size === 0)
            break;
        ...
        for (const m of fetched.values()) {
            if (m.createdTimestamp < fourteenDaysAgo)
                continue;
            if (filterArg === 'bot' && !m.author.bot)
                continue;
            if (filterArg === 'human' && m.author.bot)
                continue;
            if (filterArg === 'embeds' && m.embeds.length === 0)
                continue;
            if (filterArg === 'links' && !/(https?:\/\/[^\s]+)/g.test(m.content))
                continue;
            if (filterArg === 'images' && m.attachments.size === 0)
                continue;
            if (targetUserId && m.author.id !== targetUserId)
                continue;
...
async function handlePurgeRole(ctx, args) {
    ...
    if (totalMembers > 10) {
        ...
        const promptPayload = ui.standard({
            ...
            text: `Are you sure you want to remove ${mentionRole(targetRole, guild)} from **${totalMembers}** members?\n\n` +
    ...
    for (let i = 0; i < membersWithRole.length; i += CHUNK_SIZE) {
        ...
        const results = await Promise.all(chunk.map(targetMember => removeRoleFromMember(guild, targetMember, targetRole, member).then(res => ({ targetMember, res }))));
    ...
    const finalPayload = ui.standard({
        ...
        text: `• **Target Role:** \`${targetRole.name}\` (${mentionRole(targetRole, guild)})\n` +
    ...
    if (statusMsg) {
        ...
        const replyMsg = await respond.success(`Purged role \`${targetRole.name}\` (${mentionRole(targetRole, guild)}) from **${removedCount}** member(s).` +
