...
import { createGamePing, updateGamePing, deleteGamePing, getGamePing, listGamePings, setGameTestChannel, getGameTestChannel, } from '../../core/database/repositories/gameRepo.js';
    ...
    async execute(ctx) {
        ...
        switch (subcommand) {
            ...
            case 'test':
                ...
                break;
            default:
                await respond.error(`Unknown subcommand \`${subcommand}\`. Valid options: \`create\`, \`edit\`, \`delete\`, \`list\`, \`testchannel\`.`);
                break;
        }
    },
});
async function handleCreate(ctx, args) {
    ...
    await respond.success(`Created ping configuration ${inlineCode(identifier)}:\nGame: ${bold(gameName)}\nRole: ${mentionRole(roleId, guild)}\nVC: ${mentionChannel(vcId)}\nCooldown: ${cooldownSeconds}s`);
}
async function handleEdit(ctx, args) {
    const { guild, respond } = ctx;
    if (args.length < 4) {
    ...
    await respond.success(`Updated ping configuration ${inlineCode(identifier)}:\nGame: ${bold(gameName)}\nRole: ${mentionRole(roleId, guild)}\nVC: ${mentionChannel(vcId)}\nCooldown: ${cooldownSeconds}s`);
}
...
async function handleTestChannel(ctx, args) {
    const { guild, respond } = ctx;
    if (args.length === 0) {
    ...
    await respond.success(`Game test channel set to ${mentionChannel(targetChannelId)}. Commands run in this channel will have **no cooldown**.`);
}
