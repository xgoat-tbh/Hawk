...
const activeRequests = new Map(); // id -> FmvState
const FMV_TTL = 60_000; // 60 seconds
function scheduleMessageDeletion(msg, delayMs = 5000) {
...
export function getActiveFmvRequest(guildId, targetId) {
    return activeRequests.get(`${guildId}_${targetId}`);
}
export function getAllActiveFmvRequests() {
    return Array.from(activeRequests.values());
}
export function clearFmvStateCache() {
    for (const req of activeRequests.values()) {
        if (req.countdownTimer)
            clearTimeout(req.countdownTimer);
        if (req.expireTimer)
            clearTimeout(req.expireTimer);
    }
...
export async function createFmvRequest(options) {
    ...
    state.expireTimer = setTimeout(async () => {
        ...
        try {
            ...
            await state.requestMessage.edit({ components: payload.components, flags: payload.flags });
    ...
    if (targetId) {
        ...
        if (req && (req.authorId === authorId || isElevated)) {
            ...
            try {
                ...
                await req.requestMessage.edit({ components: payload.components, flags: payload.flags });
...
export async function handleFmvVoiceStateUpdate(oldState, newState) {
    ...
    if (!req)
        return;
    if (req.status !== 'WAITING_FOR_VC' && req.status !== 'COUNTDOWN')
        return;
    ...
    try {
        ...
        if (!destChannel || !destChannel.isVoiceBased()) {
            ...
            await req.requestMessage.edit({ components: failPayload.components, flags: failPayload.flags }).catch(() => { });
            scheduleMessageDeletion(req.requestMessage, 5000);
            return;
        }
...
//# sourceMappingURL=FmvManager.js.map