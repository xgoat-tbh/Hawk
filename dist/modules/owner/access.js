import { PermissionsBitField, ActionRowBuilder, ButtonBuilder, ButtonStyle, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, } from 'discord.js';
import { defineCommand } from '../../types/command.js';
import { resolveUser } from '../../core/resolver/UserResolver.js';
import { resolveRole } from '../../core/resolver/RoleResolver.js';
import { resolveCommand, getModules } from '../../core/commands/CommandRegistry.js';
import { addPermit, removePermit, getPermitsForGuild, deletePermitsByIds, removeAllPermitsForTarget, } from '../../core/database/repositories/permissionRepo.js';
import { grantDashboardAccess, revokeDashboardAccess, } from '../../core/database/repositories/dashboardAccessRepo.js';
...
export default defineCommand({
    ...
    async execute(ctx) {
        ...
        if (firstArg === 'list' || firstArg === 'show') {
            ...
            const fetchUserNames = async () => {
                ...
                if (uncachedUserIds.length > 0) {
                    await Promise.all(Array.from(new Set(uncachedUserIds)).map(async (uid) => {
            ...
            const PAGE_SIZE = 6;
            let currentPage = 0;
            const buildListPagePayload = (page) => {
                ...
                if (totalPages > 1) {
                    ...
                    buttonRow = new ActionRowBuilder().addComponents(prevBtn, countBtn, nextBtn);
                }
                ...
                const selectOptions = sortedEntries.slice(0, 25).map((g) => {
            ...
            collector.on('collect', async (interaction) => {
                ...
                const renderTargetInspection = async (targetType, targetId) => {
                    ...
                    const actionRow = new ActionRowBuilder().addComponents(backBtn, revokeBtn);
                    return ui.standard({
                ...
                if (interaction.isButton()) {
                    ...
                    if (interaction.customId.startsWith('access_confirm_revoke:')) {
                        ...
                        const targetType = parts[1];
                        const targetId = parts[2];
                        const removedCount = await removeAllPermitsForTarget(guild.id, targetType, targetId, member.id, sanitize(member.displayName || member.user.tag));
                        logAuditAction({
                            guild,
                        ...
                        await interaction.update({
                            ...
                            flags: revokeConfirmPayload.flags,
                        });
                        return;
                    }
                }
        ...
        if (remainingArgs.length < 2) {
            await respond.error(`Usage: \`access ${isRemoveMode ? 'remove' : 'add'} <@user|@role> <command|module|all>\``);
            return;
        }
