...
import { setSuggestionChannel, getSuggestionChannel, setSuggestionPanelMessageId, addBlacklist, removeBlacklist, listBlacklist, resetSuggestionDataForGuild, updateSuggestionStatus, updateSuggestionMessageId, } from '../../core/database/repositories/suggestionRepo.js';
...
async function handleStatusUpdate(ctx, newStatus, title) {
    ...
    const isFirstArgTarget = firstArg && ((!message.reference && /^\d+$/.test(firstArg)) ||
        firstArg.startsWith('#') ||
    ...
    if (channel) {
        ...
        if (!msg) {
            ...
            if (recentMsgs) {
                const match = recentMsgs.find(m => m.author.id === guild.client.user?.id &&
...
async function handleBlacklist(ctx, args) {
    const { guild, respond, member } = ctx;
    if (args.length === 0) {
...
async function handleReset(ctx, args) {
    ...
    if (args.length === 0 || args[0].toLowerCase() !== 'confirm') {
        await respond.warning(`⚠️ ${bold('DESTRUCTIVE OPERATION')}: Resetting suggestions will delete all suggestion history, vote records, suggestion counters, and blacklist data for this server.\n\nTo confirm, run: \`${ctx.parsed.prefix}suggestion reset confirm\``);
        return;
    }
