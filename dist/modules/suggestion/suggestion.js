...
async function handleReset(ctx, args) {
    ...
    if (args.length === 0 || args[0].toLowerCase() !== 'confirm') {
        await respond.warning(`${bold('DESTRUCTIVE OPERATION')}: Resetting suggestions will delete all suggestion history, vote records, suggestion counters, and blacklist data for this server.\n\nTo confirm, run: \`${ctx.parsed.prefix}suggestion reset confirm\``);
        return;
    }
