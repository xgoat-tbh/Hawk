...
export function buildStorePayload(items, currencySymbol, guildName, page, totalPages, invokerId) {
    let text = '';
    if (items.length === 0) {
...
export function buildItemInfoPayload(item, currencySymbol) {
    let content = `• **Item ID:** \`${item.itemId}\`\n` +
        `• **Item Name:** **${item.name}**\n` +
...
export function buildInventoryPayload(entries, userName, avatarUrl, currencySymbol) {
    let text = '';
    if (entries.length === 0) {
