...
export default defineCommand({
  ...
  async execute(ctx: CommandContext): Promise<void> {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        { name: 'Total Cash in Circulation', value: `${config.currencySymbol}${totalCash.toLocaleString()}`, inline: true },
        { name: 'Total Bank Holdings', value: `${config.currencySymbol}${totalBank.toLocaleString()}`, inline: true },
        { name: 'Total Economy Size', value: `${config.currencySymbol}${totalCirculation.toLocaleString()}`, inline: true },
        { name: 'Participating Members', value: `${memberCount.toLocaleString()}`, inline: true }
      );
