...
export default defineCommand({
  ...
  module: 'moderation',
  description: 'Server deafen one or multiple members in voice, or all members in a voice channel.',
  usage: 'vcdeafen <targets...> | vcdeafen all [channel]',
  examples: ['vcdeafen @User', 'vcdeafen @User1 @User2', 'vcdeafen all', 'vcdeafen all #General'],
  ...
  async execute(ctx: CommandContext): Promise<void> {
    ...
    if (parsed.args.length === 0) {
      await respond.error(`Usage: \`${parsed.prefix}vcdeafen <targets...> | ${parsed.prefix}vcdeafen all [channel]\``);
      return;
    }
    ...
    if (isAll) {
      ...
      if (!targetVc) {
        await respond.error('You must be connected to a voice channel or specify a voice channel to deafen all.');
        return;
      }
      ...
      const eligibleMembers = connectedMembers.filter((m) => {
        ...
        if (m.voice.serverDeaf) return false;
        return true;
      });

      if (eligibleMembers.length === 0) {
        await respond.info(`No undeafened manageable members found in ${mentionChannel(targetVc.id)}.`);
        return;
      }
      ...
      const results = await Promise.allSettled(
        eligibleMembers.map((m) => m.voice.setDeaf(true, `VCDeafen All by ${member.user.tag}`)),
      );
      ...
      await respond.success(
        `Deafened **${successCount}** user(s) in ${mentionChannel(targetVc.id)}.${failCount > 0 ? ` (Failed: ${failCount})` : ''}`,
      );

      logEvent('info', 'command_execution', `VCDeafen All by ${member.user.tag}`, {
