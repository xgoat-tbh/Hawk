...
export default defineCommand({
  ...
  module: 'moderation',
  description: 'Server unmute one or multiple members in voice, or all members in a voice channel.',
  usage: 'vcunmute <targets...> | vcunmute all [channel]',
  examples: ['vcunmute @User', 'vcunmute @User1 @User2', 'vcunmute all', 'vcunmute all #General'],
  ...
  async execute(ctx: CommandContext): Promise<void> {
    ...
    if (parsed.args.length === 0) {
      await respond.error(`Usage: \`${parsed.prefix}vcunmute <targets...> | ${parsed.prefix}vcunmute all [channel]\``);
      return;
    }
    ...
    if (isAll) {
      ...
      if (!targetVc) {
        await respond.error('You must be connected to a voice channel or specify a voice channel to unmute all.');
        return;
      }
      ...
      const eligibleMembers = connectedMembers.filter((m) => {
        ...
        if (!m.voice.serverMute) return false; // already unmuted
        return true;
      });

      if (eligibleMembers.length === 0) {
        await respond.info(`No muted manageable members found in ${mentionChannel(targetVc.id)}.`);
        return;
      }
      ...
      const results = await Promise.allSettled(
        eligibleMembers.map((m) => m.voice.setMute(false, `VCUnmute All by ${member.user.tag}`)),
      );
      ...
      await respond.success(
        `Unmuted **${successCount}** user(s) in ${mentionChannel(targetVc.id)}.${failCount > 0 ? ` (Failed: ${failCount})` : ''}`,
      );

      logEvent('info', 'command_execution', `VCUnmute All by ${member.user.tag}`, {
    ...
    await respond.success(`Unmuted **${successCount}** user(s) in voice.${failCount > 0 ? ` Failed/Not in VC: ${failCount}` : ''}`);

    logEvent('info', 'command_execution', `VCUnmute by ${member.user.tag}`, {
      executor: member.user.tag,
      executorId: member.id,
      guild: guild.name,
      guildId: guild.id,
