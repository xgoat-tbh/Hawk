import { PermissionsBitField, type Message, type TextChannel } from 'discord.js';
...
const userMessageHistory = new Map<string, MessageRecord[]>();
...
export async function handleAntiSpam(message: Message): Promise<void> {
  ...
  if (env.botOwnerIds.includes(userId) || userId === env.botOwnerId || env.botAdminIds.includes(userId)) return;
  ...
  try {
    ...
    logAuditAction({
      ...
      channelName: (textChannel as any).name ?? textChannel.id,
      details: [
