import { Client, ChannelType, PermissionsBitField, type TextChannel, type Message } from 'discord.js';
import { consoleLog } from '../../core/logging/ConsoleLogger.js';
import { logEvent } from '../../core/logging/WebhookLogger.js';
import { logAuditAction } from '../../core/logging/AuditLogger.js';

export async function runStartupSpamCleanup(client: Client): Promise<void> {
    ...
    try {
      ...
      for (const [, channel] of channels) {
        ...
        try {
          ...
          const eligibleMessages = fetched.filter((m: Message) => !m.author.bot && !m.system && m.createdTimestamp > fourteenDaysAgo);
          if (eligibleMessages.size === 0) continue;

          // Group eligible messages by author ID
          const userMessagesMap = new Map<string, Message[]>();
          ...
          const spamIdsToDelete = new Set<string>();
